...
export default function DriverDashboard() {
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20 mx-auto max-w-md relative shadow-2xl overflow-hidden border-x border-slate-200">
      ...
      {/* --- Top Mobile App Bar --- */}
      <header className="bg-blue-600 text-white px-5 py-4 shadow-md sticky top-0 z-10 flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold tracking-widest">SHERVICE</h1>
          <p className="text-blue-100 text-[10px] uppercase tracking-wider">Driver Portal</p>
        </div>
      ...
      <main className="p-4 space-y-4">
        ...
        <div className="flex justify-between items-end mb-2 px-1">
          ...
          <div className={`px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wide flex items-center gap-1.5 shadow-sm ${isClockedIn ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-white text-slate-500 border border-slate-200'}`}>
            <span className={`w-2 h-2 rounded-full ${isClockedIn ? 'bg-blue-500 animate-pulse' : 'bg-slate-300'}`}></span>
      ...
      <nav className="absolute bottom-0 w-full bg-white border-t border-slate-200 flex justify-around items-center pb-safe pt-2 px-2 h-16 z-20">
        <button className="flex flex-col items-center p-2 text-blue-600 transition-colors">
