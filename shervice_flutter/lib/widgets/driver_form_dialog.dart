    ...
    try {
      ...
      if (isEdit) {
        ...
        if (_passwordController.text.isNotEmpty) {
          ...
          if (passResponse.statusCode != 200 || passData['success'] != true) {
            throw Exception(passData['message'] ?? "Failed to override driver password.");
          }
        }
      } else {
      ...
      if ((res.statusCode == 200 || res.statusCode == 201) && responseData['success'] != false) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Driver profile saved successfully!"), backgroundColor: Color(0xFF10B981), behavior: SnackBarBehavior.floating),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("Error: ${responseData['message'] ?? 'Server validation error.'}"), backgroundColor: const Color(0xFFEF4444), behavior: SnackBarBehavior.floating),
        );
      }
    } catch (e) {
      debugPrint("❌ Form submission exception: $e");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Network error: $e"), backgroundColor: const Color(0xFFEF4444), behavior: SnackBarBehavior.floating),
      );
    } finally {
                  ...
                  child: Column(
                    ...
                    children: [
                      TextFormField(
                        ...
                        decoration: _fieldStyle(context: context, label: 'Full Name', icon: Icons.person),
                      ...
                      TextFormField(
                        ...
                        readOnly: !_isWritingUnlocked,
                        style: TextStyle(color: inputTextColor),
                        decoration: _fieldStyle(context: context, label: 'License Number', icon: Icons.card_membership),
                        validator: (v) => v == null || v.isEmpty ? 'Required' : null,
                      ),
                      const SizedBox(height: 14),
                      TextFormField(
                        ...
                        readOnly: !_isWritingUnlocked,
                        style: TextStyle(color: inputTextColor),
                        decoration: _fieldStyle(context: context, label: 'Account Email', icon: Icons.email),
                        validator: (v) => v == null || v.isEmpty ? 'Required' : null,
                      ),
                      const SizedBox(height: 14),
                      TextFormField(
                        ...
                        readOnly: true,
                        style: TextStyle(color: inputTextColor),
                        decoration: _fieldStyle(context: context, label: 'Date of Birth (YYYY-MM-DD)', icon: Icons.cake),
                        onTap: !_isWritingUnlocked ? null : () => _selectDate(_birthdayController),
                      ),
                      const SizedBox(height: 14),
                      TextFormField(
                        ...
                        decoration: _fieldStyle(
                          context: context,
                          label: isEdit ? 'Reset Password (Leave empty to keep current)' : 'Account Password',
                          icon: Icons.lock_reset,
                        ),
                        validator: (v) {
                          if (!isEdit && (v == null || v.length < 6)) return 'Password must be >= 6 chars';
                          if (isEdit && v != null && v.isNotEmpty && v.length < 6) return 'Password must be >= 6 chars';
                          return null;
                        },
                      ),
                      const SizedBox(height: 14),
                      if (isEdit) ...[
                        TextFormField(
                          ...
                          readOnly: true,
                          style: TextStyle(color: inputTextColor),
                          decoration: _fieldStyle(context: context, label: 'Date Hired', icon: Icons.event_available, forcesDisabled: true),
                        ),
                        const SizedBox(height: 14),
                        TextFormField(
                          ...
                          readOnly: true,
                          style: TextStyle(color: inputTextColor),
                          decoration: _fieldStyle(context: context, label: 'License Expiration', icon: Icons.assignment_late, forcesDisabled: true),
                        ),
                      ],
                    ],
                  ),
              ...
              children: [
                if (isEdit)
                  TextButton.icon(
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Expanded(child: DriverEvaluationView(driverUuid: widget.driver!.userId, backendUrl: widget.backendUrl)),
                              ],
                            ),
                          ),
                        ),
                    ...
                    label: const Text('View Performance', style: TextStyle(color: Color(0xFF3B82F6), fontWeight: FontWeight.bold)),
                  )
                else
                  ...
                  children: [
                    ...
                    if (isEdit && !_isWritingUnlocked)
                      ElevatedButton(
                        ...
                        child: const Text('Edit Details', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                      )
                    else
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(
                        ...
                        child: _isLoading
                            ...
                            : Text(isEdit ? 'Save Changes' : 'Register Driver', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
                      ),
                  ],
                ),
              ],
